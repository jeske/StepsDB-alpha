...
namespace Platform.Utility
{
    ...
    public class LRUCache<ItemType> where ItemType : class
    {
        ...
        {
            private LRUCache<ItemType> _owner = null;
            private Dictionary<KeyType, WeakReference> _index = null;
            private GetKeyFunc<KeyType> _getKey = null;
            private LoadItemFunc<KeyType> _loadItem = null;
            private ReaderWriterLock _lock = new ReaderWriterLock();

            /// <summary>constructor</summary>
            /// <param name="list">parent of index</param>
            ...
            public Index( LRUCache<ItemType> owner, GetKeyFunc<KeyType> getKey, LoadItemFunc<KeyType> loadItem )
            {
            ...
            {
                return RWLock.GetReadLock<INode>( _lock, _lockTimeout, delegate() {
            ...
            {
                RWLock.GetWriteLock<bool>( _lock, _lockTimeout, delegate() {
                    _index.Clear();
                    return true;
                } );
            }
            ...
            public bool AddItem( INode item )
            {
                ...
                return RWLock.GetWriteLock<bool>( _lock, _lockTimeout, delegate() {
            ...
            public int RebuildIndex()
            {
                lock( _owner._lifeSpan )
                    return RWLock.GetWriteLock<int>( _lock, _lockTimeout, delegate() {
                        _index.Clear();
        ...
        private class LifespanMgr : IEnumerable<INode>
        {
            ...
            {
                private LifespanMgr _mgr = null;
                private ItemType _value = null;
                public Node next = null;
                public AgeBag ageBag = null;
            ...
            private TimeSpan _timeSlice;
            private DateTime _nextValidCheck;
            private int _bagItemLimit;

            private AgeBag[] _bags = null;
            private AgeBag _currentBag = null;
            ...
            public LifespanMgr( LRUCache<ItemType> owner, TimeSpan minAge, TimeSpan maxAge )
            {
                ...
                _timeSlice = TimeSpan.FromMilliseconds( maxMS / 240 ); // max timeslice = 3 min
            ...
            public void CleanUp( DateTime now )
            {
                if( _current != _oldest )
                    lock( this )
                    {
                        ...
                        while( _current != _oldest && (_current-_oldest>_size - 5 || bag.startTime < maxAge || (itemsToRemove > 0 && bag.stopTime > minAge)) )
                        {
        ...
        private Dictionary<string, IIndex> _indexList = new Dictionary<string, IIndex>();
        protected IsValid _isValid = null;
        private LifespanMgr _lifeSpan = null;
        ...
        public IIndex<KeyType> AddIndex<KeyType>( String indexName, GetKeyFunc<KeyType> getKey, LoadItemFunc<KeyType> loadItem )
        {
            Index<KeyType> index = new Index<KeyType>( this, getKey, loadItem );
