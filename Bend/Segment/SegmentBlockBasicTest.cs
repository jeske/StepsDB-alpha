            ...
            foreach (int key_part_count in num_levels) {
                ...
                foreach (int block_size in block_sizes) {
                    foreach (int value_size in value_sizes) {                        
                        ...
                        System.Console.WriteLine("BlockSize {0,10}, ValueSize {1,10}, Keyparts {5,10}, {2,10} reads in {3,10}ms,  {4,10} read/sec",
                            block_size, value_size, READ_COUNT, duration_ms, reads_per_second, key_part_count);
                    }
                }
            }
